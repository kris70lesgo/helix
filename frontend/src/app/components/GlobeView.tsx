...
type ApiSatellitePosition = Omit<SatPoint, "riskLevel">;
...
export function GlobeView({ selectedNoradId, onSelectSatellite }: Props) {
  ...
  const fetchPositions = useCallback(
    async (conjEvents: ConjunctionEvent[]) => {
      try {
        ...
        const satellites = (data.satellites ?? []) as ApiSatellitePosition[];
        const posById: Record<string, { lat: number; lon: number }> = {};
        for (const s of satellites) posById[s.norad_id] = s;
  ...
  useEffect(() => {
    ...
    const loadOrbit = async () => {
      ...
      try {
        const r = await fetch(`${API}/orbit/${selectedNoradId}?hours=2&step=30`);
        if (!r.ok) {
          if (!cancelled) setOrbitTrack([]);
          return;
        }
        const data = await r.json();
        if (!cancelled) setOrbitTrack(splitOrbitTrack(data.track ?? []));
      } catch {
        if (!cancelled) setOrbitTrack([]);
      }
    };
  ...
  const lodPoints = useMemo(() => {
    ...
    const camAlt = cameraAltitude;
    const camLat = cameraPos.lat;
    const camLng = cameraPos.lng;
  ...
  const pointRadius = useMemo(() => {
    ...
    return Math.max(0.12, Math.min(0.35, 0.15 + cameraAltitude * 0.08));
  }, [cameraAltitude]);
  ...
  const dangerRings = useMemo<RingDatum[]>(() => {
    return satPoints
